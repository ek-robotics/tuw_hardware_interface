...
#include "../../../include/tuw_dynamixel_hardware_interface/description/dynamixel_joint_description.h"
...
DynamixelJointDescription::DynamixelJointDescription(const YAML::Node& yaml) : GenericJointDescription(yaml)
{
  ...
  if (yaml["connection"].IsDefined())
    this->connection_description_ = std::make_shared<DynamixelConnectionDescription>(yaml["connection"]);
  else
    ROS_ERROR("[%s] missing connection description", PREFIX);

  if (yaml["hardware"].IsDefined())
    this->hardware_description_ = std::make_shared<tuw_ros_control_generic::GenericHardwareDescription>(loadFile(yaml["hardware"]));
  else
    ROS_ERROR("[%s] missing hardware description", PREFIX);

  if (yaml["config"].IsDefined())
    this->config_description_ = std::make_shared<tuw_ros_control_generic::GenericConfigDescription>(loadFile(yaml["config"]));
  else
